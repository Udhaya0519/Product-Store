...
export async function runModeMigrate({ args, configFile, directory = ".", display, from, help, offline, preset: requestedPreset, yes, }) {
    ...
    const templateLocator = preset.base.template &&
        (await getForkedTemplateLocator(directory, preset.base.template));
    if (templateLocator) {
        await runSpinnerTask(display, `Clearing from ${templateLocator}`, `Cleared from ${templateLocator}`, async () => {
    ...
    const creation = await runSpinnerTask(display, `Running the ${preset.about.name} preset`, `Ran the ${preset.about.name} preset`, async () => await runPreset(preset, {
        ...mergedSettings,
        ...system,
        directory,
        mode: "migrate",
